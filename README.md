...
> **Portfolio adaptation:** This fork keeps the original production architecture intact and broadens the insurance domain to include digital-first **home, car, and pet insurance** recommendation flows. The goal is to better represent a modern insurtech product portfolio while preserving the existing Google ADK → MCP Toolbox → SQL/RAG → FastAPI/Cloud Run design.
...
This repository already includes a production-style **Next.js web application**, so no extra Streamlit/Gradio layer is required. The customer-facing experience is now the default view and is also available through a dedicated dashboard URL.

### Run the complete demo locally

```bash
cp .env.example .env
# Fill in the required Google/Gemini credentials in .env
...
- **FastAPI health check:** http://localhost:8080/healthz

The dashboard reuses the existing recommendation UI and backend pipeline:
...
SQL product   FAQ / RAG
   tools        search
       ↘      ↙
        Gemini
          ↓
Recommendation cards + explanation
```

This keeps the original backend, agent, database, RAG, authentication, Docker and cloud-deployment structure intact while giving the project a clear browser-based portfolio entry point.

For the seeded local Docker demo, you can use the existing development account:
...
These credentials are for local/demo use only; replace them before any public deployment.

## Simple resume demo (Gradio)

A standalone portfolio dashboard is available in `portfolio_demo.py`. It intentionally keeps the full Next.js application untouched and provides a one-file browser demo that is easier to host and link from a resume.
...
Open `http://localhost:7860`. To have Gradio request a temporary public share URL while the process is running:

```bash
GRADIO_SHARE=true python portfolio_demo.py
```

For a stable resume URL, use `portfolio_demo.py` as the app entry point on a Gradio-compatible host. The dashboard itself has no database requirement; the production ADK/FastAPI application remains unchanged elsewhere in this repository.
